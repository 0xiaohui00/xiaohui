...
GameScene::GameScene(void)   //游戏初始化
{
	mSpeed = 3;
	isPlaying = false;
	Score = 0;
...
// on "init" 
Scene* GameScene::createScene()    //在游戏场景中添加一个图层
{
...
bool GameScene::init()
{
	...
	auto pauseItem = MenuItemImage::create(
	   ...
	   pauseItem->setPosition(Vec2(pauseItem->getContentSize().width / 2 + 5,
		...
		restartsItem = MenuItemImage::create(
			"start.png",
			"start.png",
			CC_CALLBACK_1(GameScene::menurestartsCallback, this));

		restartsItem->setPosition(Vec2(winSize.width / 2, winSize.height / 2));
		 overbgItem = MenuItemImage::create(
			"overbg.png",
			"overbg.png",
			CC_CALLBACK_1(GameScene::menuoverbgCallback, this));

		overbgItem->setPosition(Vec2(winSize.width / 2, winSize.height / 2));
		...
		exitItem->setPosition(Vec2(winSize.width / 2 , winSize.height / 2 - 100));
		
	
		auto menu = Menu::create(pauseItem, restartsItem,overbgItem,exitItem, NULL);
	    menu->setPosition(Vec2::ZERO);
	   this->addChild(menu, 3);

	bg1 = Sprite::create("bg.jpg");  
	//bg1->setAnchorPoint(Vec2(0,0));
	bg1->setPosition(Vec2(visibleSize.width / 2 , visibleSize.height / 2));
	this->addChild(bg1, 1);  //设为最底层

	bg2 = Sprite::create("bg.jpg"); 
	bg2->setPosition(Vec2(visibleSize.width / 2, bg1->getContentSize().height ));
	this->addChild(bg2, 1);

	Road = Sprite::create("road.jpg");  //设置公路
	Road->setAnchorPoint(Vec2(0, 0));
	Road->setPosition(Vec2(visibleSize.width / 4 -30, 0));
	...
	this->scheduleOnce(schedule_selector(GameScene::Start),0.1f);
	...
	for (int x = 0; x<10; x++)
	{
		...
		roles[x]->setPosition(Vec2((visibleSize.width / 2 - Road->getContentSize().width / 2)  + left + 30, high));
...
void GameScene::Start(float t)
{
	...
	for (int i = 0; i<10; i++)
	{
		...
		int left = rand() % 350;
		int high = 680 + rand() % 15000;
		roles[i]->setPosition(Vec2((visibleSize.width / 2 - Road->getContentSize().width / 2)  + left + 30, high));
...
void GameScene::menurestartsCallback(cocos2d::Ref* pSender)
{	
	this->scheduleOnce(schedule_selector(GameScene::Start), 0.01f);
}
void GameScene::menuoverbgCallback(cocos2d::Ref* pSender){
...
{   
	...
	this->schedule(schedule_selector(GameScene::updateSel), 1);//自定义调度器
	return true;
} 
...
void GameScene::updateSel(float t)
{
